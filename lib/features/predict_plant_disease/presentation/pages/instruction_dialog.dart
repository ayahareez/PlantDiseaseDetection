...
Future<void> showInstructionsDialog(BuildContext context, void Function(void Function()) setState) async {
  File? imageFile;
  String? imageUrl;
  bool flag = false;
            ...
            content: const SingleChildScrollView(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Text('- Ensure there is no clutter in the background to help the model focus on the leaf.'),
                  SizedBox(height: 10),
                  Text('3. No Overlapping Leaves:'),
                  Text('- Do not capture overlapping leaves; the model works best with a single, isolated leaf.'),
                  SizedBox(height: 10),
                  Text('4. Corn Plant Specifics:'),
                  Text('- If you are capturing a corn plant, focus on capturing a part of the leaf, not the entire leaf.'),
                  SizedBox(height: 10),
                  Text('5. Good Lighting:'),
                  Text('- Ensure there is sufficient lighting for a clear image.'),
                  SizedBox(height: 10),
                  Text('6. No Distortions:'),
                  Text('- Avoid any distortions in the image; a straightforward picture works best.'),
                  SizedBox(height: 10),
                  Text('7. High-Quality Image:'),
                  Text('- Capture the image in high resolution for accurate disease prediction.'),
                ],
              ),
            ),
            actions: <Widget>[
